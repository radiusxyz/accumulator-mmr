...
impl<'a, T: Clone + PartialEq + Debug, M: Merge<Item = T>, S: MMRStore<T>> MMR<T, M, S> {
  ...
  pub fn get_root(&self) -> Result<T> {
    ...
    let peaks: Vec<T> = get_peaks(self.mmr_size)
      .into_iter()
      .map(|peak_pos| self.batch.get_elem(peak_pos).and_then(|elem| elem.ok_or(Error::InconsistentStore)))
  ...
  fn gen_proof_for_peak(&self, proof: &mut Vec<T>, pos_list: Vec<u64>, peak_pos: u64) -> Result<()> {
    ...
    if pos_list.is_empty() {
      ...
      proof.push(self.batch.get_elem(peak_pos)?.ok_or(Error::InconsistentStore)?);
      return Ok(());
    }
    ...
    while let Some((pos, height)) = queue.pop_front() {
      ...
      if Some(&sib_pos) == queue.front().map(|(pos, _)| pos) {
        ...
        proof.push(self.batch.get_elem(sib_pos)?.ok_or(Error::InconsistentStore)?);
      }
...
impl<T: PartialEq + Debug + Clone, M: Merge<Item = T>> MerkleProof<T, M> {
  ...
  pub fn calculate_root_with_new_leaf(&self, mut leaves: Vec<(u64, T)>, new_pos: u64, new_elem: T, new_mmr_size: u64) -> Result<T> {
    let pos_height = pos_height_in_tree(new_pos);
    let next_height = pos_height_in_tree(new_pos + 1);
    if next_height > pos_height {
      let mut peaks_hashes = calculate_peaks_hashes::<_, M, _>(leaves, self.mmr_size, self.proof.iter())?;
  ...
  pub fn verify(&self, root: T, leaves: Vec<(u64, T)>) -> Result<bool> {
    self.calculate_root(leaves).map(|calculated_root| calculated_root == root)
  }
}

fn calculate_peak_root<'a, T: 'a + PartialEq + Debug + Clone, M: Merge<Item = T>, I: Iterator<Item = &'a T>>(leaves: Vec<(u64, T)>, peak_pos: u64, proof_iter: &mut I) -> Result<T> {
  debug_assert!(!leaves.is_empty(), "can't be empty");
  // (position, hash, height)
  let mut queue: VecDeque<_> = leaves.into_iter().map(|(pos, item)| (pos, item, 0u32)).collect();
  ...
  while let Some((pos, item, height)) = queue.pop_front() {
    ...
    let parent_item = if next_height > height { M::merge(&sibling_item, &item) } else { M::merge(&item, &sibling_item) };
...
fn calculate_peaks_hashes<'a, T: 'a + PartialEq + Debug + Clone, M: Merge<Item = T>, I: Iterator<Item = &'a T>>(mut leaves: Vec<(u64, T)>, mmr_size: u64, mut proof_iter: I) -> Result<Vec<T>> {
...
fn bagging_peaks_hashes<'a, T: 'a + PartialEq + Debug + Clone, M: Merge<Item = T>>(mut peaks_hashes: Vec<T>) -> Result<T> {
  // bagging peaks
...
/// 3. bagging peaks
fn calculate_root<'a, T: 'a + PartialEq + Debug + Clone, M: Merge<Item = T>, I: Iterator<Item = &'a T>>(leaves: Vec<(u64, T)>, mmr_size: u64, proof_iter: I) -> Result<T> {
